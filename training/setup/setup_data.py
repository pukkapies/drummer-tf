    ...
    with tf.variable_scope("training_data"):
        for i in range(num_files):
            with tf.variable_scope('file_{}'.format(i)):
                ...
                data_block = np.hstack(loaded_data[i])
                data.append(data_block)

        zero_start = np.zeros((batch_size, 1, data_block.shape[1]))
        data = np.stack(data)  # (batch_size, n_frames, num_features)
        input_data = np.concatenate((zero_start, data[:, :-1, :]), axis=1)  # Zero vector in the beginning, remove the last frame
        output_data = data

        output_data = np.transpose(output_data, (1, 0, 2))  # (n_frames, batch_size, num_features)
        input_data = np.transpose(input_data, (1, 0, 2))  # (n_frames, batch_size, num_features)
