...
def nextbiggestpower2(num):
    """
    Computes the next biggest power of two, bigger than or equal to num
    :param num: positive integer
    :return: int, next biggest power of two
    """
    if isPower2(num):
        return num
    else:
...
def bitLen(int_type):
    """
    Returns bit length of int_type
    :param int_type: positive integer
...
def peakDetection(mX, t):
    ...
    returns ploc: indices in mX of peaks
    """
    #TODO: Might want to rewrite this function, since if there is a peak in mX with value 0, it will not be detected
...
def peakInterp(mX, pX, ploc):
    """
    Interpolate peak value indices using parabolic interpolation
    mX, pX: magnitude and phase spectrum, ploc: index locations of peaks
    returns iploc, ipmag, ipphase: interpolated peak location indices, magnitude and phase values
    """
...
def genBhLobe(x):
    ...
    returns y: main lobe of spectrum of a Blackman-Harris window
    """
...
def genSpecSines_p(ipfreq, ipmag, ipphase, N, fs):
    ...
    ipfreq, ipmag, ipphase: 1D arrays containing sine peaks frequencies, magnitudes (in dB) and phases
    ipfreq should be within the range of frequencies (0, sr/2 - sr/N) captured by an FFT of size N and sampling rate fs
    ...
    for i in range(0, ipfreq.size):  # generate all sine spectral lobes
        ...
        b = np.arange(int(round(loc)) - 4, int(round(loc)) + 5) # indices of lobe support
        for m in range(0, 9):
