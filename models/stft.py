...
def stftAnal(x, w, N, H):
    ...
    returns xmX, xpX: magnitude (in dB) and phase spectra
    """
    if (H <= 0):  # raise error if hop size 0 or negative
        raise ValueError("Hop size (H) smaller or equal to 0")
    ...
    pend = x.size - hM1  # last sample to start a frame
    w = w / sum(w)  # normalize analysis window
    while pin <= pend + 1:  # while sound pointer is smaller than last sample - NB added 1
        x1 = x[pin - hM1:pin + hM2]  # select one frame of input sound
        mX, pX = DFT.dftAnal(x1, w, N)  # compute dft
...
def stftSynth(mY, pY, M, H):
    ...
    for i in range(nFrames):  # iterate over all frames
        ...
        y[pin - hM1:pin + hM2] += H * y1  # overlap-add to generate output sound
        # (Recall that the window was normalised to w/sum(w), and assuming that the windows overlap-add
        # to a constant, then the constant must be 1/H
        pin += H  # advance sound pointer
    y = np.delete(y, range(hM2))  # delete half of first window which was added in stftAnal
