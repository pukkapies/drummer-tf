...
def dftAnal(x, w, N):
    ...
    x: input signal, w: analysis window, N: FFT size
    x and w should be the same length
    returns mX, pX: magnitude (in dB) and phase spectrum
    """
    ...
    absX[absX < np.finfo(float).eps] = np.finfo(float).eps  # if zeros add epsilon to handle log
    mX = 20 * np.log10(absX)  # magnitude spectrum of positive frequencies in dB
    X[:hN].real[np.abs(X[:hN].real) < tol] = 0.0  # for phase calculation set the small values to 0
    X[:hN].imag[np.abs(X[:hN].imag) < tol] = 0.0  # for phase calculation set the small values to 0
    pX = np.unwrap(np.angle(X[:hN]))  # unwrapped phase spectrum of positive frequencies
    return mX, pX
